...
contract SecondSwap_StepVesting is SecondSwap_Vesting {
    ...
    function claimable(address _beneficiary) public view returns (uint256, uint256) {
        ...
        if (currentTime < startTime) { // @audit-info : when time is <= 
            return (0, 0);
        }

        uint256 elapsedTime = currentTime - startTime; // -5 month
        uint256 currentStep = elapsedTime / stepDuration; // 5
        ...
        claimableAmount = vesting.releaseRate * claimableSteps; // 2*1 => 2
        return (claimableAmount, claimableSteps);
    }
    ...
    function claim(address user) internal {
            (uint256 claimableAmount, uint256 claimableSteps) = claimable(user);
            if(claimableAmount >0){
            Vesting storage vesting = _vestings[user];
            vesting.stepsClaimed += claimableSteps;
            vesting.amountClaimed += claimableAmount;

            token.safeTransfer(user, claimableAmount); //  3.6. DOS caused by the use of transfer and transferFrom functions
    ...
    function transferVesting(address _grantor, address _beneficiary, uint256 _amount) external {
        ...
        grantorVesting.releaseRate = grantorVesting.totalAmount / numOfSteps; // 6/3 => 2 => 1
        // what if user did not claim the token alllocation jab wo claim kar stka tha tha , us n listing laga di , or us ki bad us n claim kya to kya hpga??
    ...
    function _createVesting(
        ...
        if (_vestings[_beneficiary].totalAmount == 0) {
            ...
            // total token = 10 tokens , per month 1 token , 
            // 4th month it means , amountClaimed = 4 tokens
            // step 2 :
            // add 10 tokens 
            //  totalAmount = 20 tokens
            // 16 / 10-4 => 2;
